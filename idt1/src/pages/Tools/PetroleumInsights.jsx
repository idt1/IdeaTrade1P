...
export default function PetroleumInsights() {
  ...
  const featuresSection = (
    <div className="w-full mb-10">
      <h2 className="font-bold mb-6 border-l-4 border-cyan-500 pl-4" style={{ fontSize: "clamp(1.25rem, 3vw, 1.75rem)" }}>
        4 Main Features
      </h2>
      <div
      ...
      >
        <button
          ...
          className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-5 sm:-translate-x-10 z-20 w-10 h-10 rounded-2xl bg-[#0f172a]/90 border border-slate-600 text-white hover:bg-cyan-500 hover:border-cyan-400 flex items-center justify-center transition-all duration-300 active:scale-95 ${showLeft ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
        </button>
        <div ref={scrollContainerRef} onScroll={checkScroll} className="flex overflow-x-auto gap-4 py-3 px-1" style={scrollbarHideStyle}>
          {features.map((item, index) => (
            <div
              key={`feat-${index}`}
              className="flex-shrink-0 group/card bg-[#0f172a]/60 border border-slate-700/50 p-6 rounded-xl hover:bg-[#1e293b]/60 hover:border-cyan-500/30 transition duration-300"
              style={{ width: "clamp(240px, 28vw, 360px)" }}
            >
              <h3 className="text-base font-bold text-white mb-2 group-hover/card:text-cyan-400 transition-colors">{item.title}</h3>
        ...
        <button
          ...
          className={`absolute right-0 top-1/2 -translate-y-1/2 translate-x-5 sm:translate-x-10 z-20 w-10 h-10 rounded-2xl bg-[#0f172a]/90 border border-slate-600 text-white hover:bg-cyan-500 hover:border-cyan-400 flex items-center justify-center transition-all duration-300 active:scale-95 ${showRight ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}
        >
