...
function WaveSkeleton({ delay = 0 }) {
  return (
    <div className="w-full bg-[#0f172a] rounded-lg overflow-hidden relative" style={{ height: "100%" }}>
...
function ChartCard({
  ...
  return (
    ...
    <div className="relative bg-[#111827] border border-slate-700 rounded-xl flex flex-col h-full min-h-0">
      ...
      {/* Chart body — WaveSkeleton when refreshing */}
      {isRefreshing ? (
        <div className="relative flex-1 bg-[#0f172a] min-h-0 p-3">
          <WaveSkeleton delay={skeletonDelay} />
        </div>
      ) : (
...
export default function S50() {
  ...
  ========================================================= */
  const chartCards = [
    { title: "1. Last (SET50 Daily)",      chartId: "chart1", delay: 0,   hasHint: true },
    { title: "2. Confirm Up/Down S50",     chartId: "chart2", delay: 0.2, hasHint: false },
    { title: "3. Trend (Volume Flow)",     chartId: "chart3", delay: 0.4, hasHint: false },
    { title: "4. Mid-Trend (SET Context)", chartId: "chart4", delay: 0.6, hasHint: false },
  ];
  ...
  return (
    <div className="w-full h-[100dvh] overflow-y-auto bg-[#0b111a] text-white px-3 sm:px-6 py-4 sm:py-6 flex flex-col">
      ...
      <div className="max-w-[1600px] w-full mx-auto flex-1 flex flex-col min-h-0">
        ...
        >
          {chartCards.map(({ title, chartId, delay, hasHint }) => (
            <ChartCard
              key={chartId}
              title={title}
              timeframe={timeframe}
